...
Result semantics preserve deterministic rule/finding/evidence identifiers, outcome, applicability, measurement, and provenance. Collection coverage is page collection; measurement coverage is rule measurement. Methodology v2 assigns each score-eligible logical rule to one primary axis only; secondary tags and supplemental evidence do not add penalties. V1 sheet mappings remain historical compatibility data and are not a v2 fallback. Scores include applicable measured pass/fail rules only; warnings are cautions, and not-applicable, unknown, not-measured, and collection-failed items do not receive score credit. Axe is standards automation; Lighthouse values are laboratory measurements, not field Core Web Vitals, and its aggregate category is report-only. Visual-model output is probabilistic, schema-validated, and does not independently score. Historical audits, revisions, and publication snapshots are not recomputed.
...
- `PS-SCORE-AC-001`: The configured five-axis taxonomy uses applicable measured pass/fail evidence, one primary scored axis per logical rule, explicit deduplication, and measurement coverage. Evidence: `src/gtm_audit/rule_registry.py`, `src/gtm_audit/scoring.py`, `tests/test_phase3a_scoring.py`, `tests/test_methodology_v2_scoring.py`.
