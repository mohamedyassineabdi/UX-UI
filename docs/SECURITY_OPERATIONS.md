...
**Related documents:** [Technical Specification](TECHNICAL-SPECIFICATION.md), [Architecture](ARCHITECTURE.md), [Checkpoint](CHECKPOINT.md).
...
- Publication deploys an isolated temporary copy of the selected immutable machine report. Local edits are not accepted as HTML and are not published.

## External / inherited release prerequisites
...
The legacy incident path, `UX-UI_-Agent/info .txt`, is not present in the reachable history of this `UX-UI` repository. Do not run `git filter-repo`, rewrite this repository's history, or force-push merely because of the legacy incident. The CI Gitleaks job is a full-history secret-detection scanning gate for reachable repository history; it does not prove that external credential remediation is complete.
...
CSRF protection is not applicable to the current mutation API: sensitive endpoints authenticate only through an explicitly supplied `Authorization: Bearer` token. The server does not use ambient browser cookies for authorization. If cookie-backed authentication is introduced later, mutation endpoints must add CSRF protection before release.

## Infrastructure boundary

This is a conditional external-infrastructure note. The repository does not contain AWS ALB listener or routing configuration, and it does not establish that an ALB is deployed. If an internet-facing deployment uses an ALB, its owner must verify those protections independently; application bearer authentication alone does not prove them.
