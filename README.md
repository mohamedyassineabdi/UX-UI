...
## Phase 3B measurement limitations

Accessibility runtime observations are custom heuristics unless a standards automation tool confirms them. Automated checks do not constitute WCAG conformance testing; manual review remains necessary. Lighthouse 13.4.1 measurements are labelled laboratory data (Lighthouse lab performance score, Lab LCP, Lab CLS, Lab FCP, Lab TBT, and Speed Index), not PageSpeed Insights, CrUX, field Core Web Vitals, or real-user metrics; they do not modify Phase 3A compliance scoring. Visual-model findings are probabilistic, schema-validated interpretations and retain provider/model/prompt metadata. Complex contrast backgrounds and logical focus order are recorded as not measured or human-review limitations rather than fabricated certainty.
