...
_ENVIRONMENT = (os.getenv("APP_ENV") or os.getenv("UX_ENVIRONMENT") or "development").strip().lower()
if not FIGMA_VERIFY_SSL:
    if _ENVIRONMENT in {"production", "prod", "staging"}:
        raise RuntimeError("FIGMA_VERIFY_SSL=false is forbidden outside local development.")
    warnings.warn("INSECURE DEVELOPMENT FIGMA TLS OVERRIDE IS ACTIVE.", RuntimeWarning, stacklevel=1)
if _ENVIRONMENT in {"production", "prod", "staging"} and urlparse(FIGMA_API_BASE).scheme != "https":
    raise RuntimeError("FIGMA_API_BASE must use HTTPS outside local development.")
