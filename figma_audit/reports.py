...
def _recommendations_section(
    ...
    if not issues:
        body = """
        <p class="empty-state" data-reco-empty>No issue-level recommendation is shown yet.</p>
        """
    else:
        cards = []
        for index, issue in enumerate(issues, start=1):
            ...
            cards.append(
                f"""
                <article class="reco-card priority-{_html(str(issue.get("severity") or "low"))}" data-recommendation-card data-reco-index="{index}" tabindex="0">
                  <span class="reco-orb">{index:02d}</span>
                  <div class="reco-card-actions" data-local-edit-control>
                    <button type="button" data-reco-action="edit" aria-pressed="false">Edit</button>
                    <button type="button" data-reco-action="delete">Delete</button>
                  </div>
                  <span class="reco-badge" data-reco-editable="priority">{_html(str(issue.get("severity") or "draft").title())}</span>
                  <h4 data-reco-editable="title">{_html(_polished_value(polished_copy, issue, "title", _human_issue_title(issue)))}</h4>
                  <p data-reco-editable="body">{_html(_polished_value(polished_copy, issue, "recommended_fix", _recommendation(issue, criterion)))}</p>
                  <p class="reco-impact">Why this is prioritized: <span data-reco-editable="impact">{_html(_polished_value(polished_copy, issue, "why_it_matters", _why_it_matters(issue, criterion)))}</span></p>
                  <span class="reco-axis" data-reco-editable="axis">{_html(_criterion_name(criterion_id, lookup))}</span>
                  <span class="reco-cta" data-reco-editable="cta">Recommended fix</span>
                </article>
                """
            )
        body = "".join(cards)
    ...
    <section class="section-panel" id="recommendations">
      <div class="section-head">
        ...
        <div class="reco-section-actions" data-local-edit-control>
          <button type="button" data-reco-action="add">Add recommendation</button>
        </div>
      </div>
      <div class="reco-grid" data-reco-list>{body}</div>
    </section>
    """
...
def _style() -> str:
    ...
    * { box-sizing: border-box; }
    ...
    .reco-section-actions {
      display: flex;
      justify-content: flex-end;
      align-items: center;
    }
    .reco-section-actions button {
      background: var(--ink);
    ...
    .reco-card-actions button {
      padding: 6px 9px;
      font-size: 0.76rem;
      line-height: 1;
    }
    .reco-card-actions button[aria-pressed="true"] {
      background: var(--teal);
      border-color: var(--teal);
      color: #fff;
    }
    ...
    .booking-section {
      ...
      border: 1px solid rgba(198,161,55,0.36);
      border-radius: 8px;
      background:
        linear-gradient(135deg, rgba(255,225,0,0.26), rgba(255,255,255,0.88) 38%, rgba(17,136,110,0.12)),
        #fffdf6;
      box-shadow: 0 22px 54px rgba(32,39,51,0.10);
    }
    .booking-copy {
      display: grid;
      gap: 10px;
    ...
    .booking-button {
      ...
      box-shadow: 0 16px 30px rgba(32,39,51,0.18);
    }
    ...
    @media (max-width: 640px) {
      ...
      .hero-meta { gap: 16px; }
      .booking-section { grid-template-columns: 1fr; padding: 22px; }
      .booking-copy h2 { max-width: 100%; }
...
def _review_script(
  ...
  (() => {
    ...
    function recommendationTemplate() {
      return `
        <article class="reco-card priority-custom" data-recommendation-card tabindex="0">
          ...
          <h4 data-reco-editable="title">New recommendation</h4>
          <p data-reco-editable="body">Describe the recommendation and the expected user experience improvement.</p>
          <p class="reco-impact">Why this is prioritized: <span data-reco-editable="impact">Expected impact for the audited flow.</span></p>
    ...
    function ensureRecommendationEmptyState(list) {
      ...
      if (!hasCards && !empty) {
        ...
        message.textContent = "No issue-level recommendation is shown yet.";
    ...
    const embeddedAuditId = document.querySelector('meta[name="uxui-audit-id"]')?.content || "";
    ...
    const canDeployFromHere = (isAuditPath || /^[a-f0-9]{12}$/.test(embeddedAuditId)) && !isKnownStaticDeployment;
    ...
    async function publishTrustedReport() {
      const match = window.location.pathname.match(/[/]audits[/]([a-f0-9]{12})[/]/);
      const auditId = match ? match[1] : document.querySelector('meta[name="uxui-audit-id"]')?.content;
      if (!auditId || !/^[a-f0-9]{12}$/.test(auditId)) throw new Error("Could not identify this audit report.");
      const token = sessionStorage.getItem("internalPortalToken") || sessionStorage.getItem("internalPortalAdminToken");
      if (!token) throw new Error("Your portal session is missing. Sign in again.");
      const endpoint = `${new URL(document.baseURI).origin}/api/audits/${auditId}/publish`;
      ...
      if (!response.ok) throw new Error(payload.error || `Publication failed with status ${response.status}.`);
      return payload;
    }
    ...
    if (deployButton) {
      ...
      deployButton.addEventListener("click", async () => {
        ...
        deployButton.disabled = true;
        if (deployStatus) deployStatus.textContent = "Publishing the immutable machine-generated report...";
        try {
          const payload = await publishTrustedReport();
          if (deployStatus) {
...
def build_detection_review_report(
    *,
    ...
    booking_subject = f"Free 30-minute UX/UI session for {file_label}"
    booking_body = (
        f"Hello Yassine,\n\n"
        f"I reviewed the UX/UI audit for {file_label} and would like to book the free 30-minute session with a UX/UI expert.\n\n"
        f"Audit subject: Figma UX/UI audit\n"
        f"Company / project: {file_label}\n"
    ...
    <template data-deployed-booking-template>
      <section class="booking-section" id="book-session" aria-labelledby="book-session-title">
        <div class="booking-copy">
          ...
          <p>Discuss the priority findings in this audit with a UX/UI expert and turn the recommendations into a practical action plan for {_html(file_label)}.</p>
        ...
        <a class="booking-button" href="{_html(booking_href)}" target="_blank" rel="noopener">Book a session</a>
      </section>
    </template>
